...
|--------|----------|--------------|
| [Проект 1: ML сервис рекомендательной системы](https://github.com/anastasia-sharina/projects/tree/main/projects_rus/Analytics_Projects_rus/1_Nashville_Housing_Project_rus) | ML-сервис реализует классическую рекомендательную систему для социальных постов. В основе лежит модель CatBoost, обученная на пользовательских и постовых признаках, включая текстовые эмбеддинги, категориальные и временные фичи, а также кластерные признаки. Сервис предоставляет API для получения персональных рекомендаций через FastAPI. | Python 3.8+, CatBoost, scikit-learn, NLTK, Pandas, NumPy, FastAPI, SQLAlchemy, PostgreSQL, Docker, Seaborn, Matplotlib (feature analysis) |
| [Проект 2: DL сервис рекомендательной системы](https://github.com/anastasia-sharina/projects/tree/main/projects_rus/Analytics_Projects_rus/2_COVID19_Project_rus) | DL-сервис реализует рекомендательную систему на базе современных трансформеров (BERT, RoBERTa, DistilBERT) для генерации эмбеддингов текстов постов. Кластеризация эмбеддингов и вычисление кластерных расстояний используются как дополнительные признаки для модели CatBoost. Сервис предоставляет API для получения рекомендаций через FastAPI. | Python 3.8+, PyTorch, Transformers (HuggingFace), CatBoost, scikit-learn, Pandas, NumPy, FastAPI, SQLAlchemy, PostgreSQL, Docker, tqdm (inference) |
